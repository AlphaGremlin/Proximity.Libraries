...
namespace Proximity.Utility.Threading
{
	...
	{	//****************************************
		...
		{
			counter.PeekDecrement().ContinueWith(OnPeekCompleted, counter, _TokenSource.Token);
		}
			...
			if (task.IsFaulted)
			{
				lock (this)
				{
					...
					if (task.Exception.InnerException is ObjectDisposedException)
						...
						SetException(new ApplicationException("Failed to peek any counters", task.Exception.InnerException));
