...
namespace Proximity.Utility.Reflection
{
	...
	{	//****************************************
		...
		{	//****************************************
			...
			foreach(var MyField in MyType.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
			{
				if (MyField.FieldType == typeof(int) || MyField.FieldType == typeof(long))
				{
					MyEmitter
						.LdArg(0) // Left
