...
namespace Proximity.Utility.Collections
{
	...
	{	//****************************************
		...
		{
			counter.PeekDecrement(_TokenSource.Token).ContinueWith(OnCounterPeek, collection, TaskContinuationOptions.NotOnCanceled);
		}
			...
			if (task.IsFaulted)
			{
				lock (this)
				{
					...
					if (task.Exception.InnerException is ObjectDisposedException)
						...
						SetException(new ApplicationException("Failed to decrement any counters", task.Exception.InnerException));
				}
