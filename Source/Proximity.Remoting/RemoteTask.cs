...
namespace Proximity.Remoting
{
	...
	{ //****************************************
		...
		/// <inheritdoc />
		[SecurityCritical]
		public override object InitializeLifetimeService() => null; // Last until the Task we're attached to is completed
		...
		{ //****************************************
			var Source = (RemoteTaskCompletionSource)state;
			//****************************************
		...
		public static RemoteTask Start(Func<Task> callback) => new RemoteTask(callback());
		...
		/// <param name="callback">The callback returning a task to wrap</param>
		/// <returns>A remote task to pass across AppDomain boundaries</returns>
		public static RemoteTask<TResult> Start<TResult>(Func<Task<TResult>> callback) => new RemoteTask<TResult>(callback());
		...
		public static implicit operator Task(RemoteTask remoteTask) => CreateTask(remoteTask);
		...
		public static implicit operator RemoteTask(Task task) => new RemoteTask(task);

		/// <summary>
		/// Creates a Remote Task that has already completed with no result
		/// </summary>
		/// <returns>A completed Remote Task</returns>
		public static RemoteTask FromResult() => new RemoteTask(Task.CompletedTask);
		...
		public static RemoteTask<TResult> FromResult<TResult>(TResult result) => new RemoteTask<TResult>(Task.FromResult(result));
		...
		internal static Task CreateTask(RemoteTask remoteTask)
		{
			...
			var TaskSource = new RemoteTaskCompletionSource(remoteTask);
	...
	{//****************************************
		...
		/// <param name="callback">The callback returning a task to wrap</param>
		/// <returns>A remote task to pass across AppDomain boundaries</returns>
		public static RemoteTask<TResult> Start(Func<Task<TResult>> callback) => new RemoteTask<TResult>(callback());
		...
		/// <returns>A task that will reflect the status of the RemoteTask</returns>
		/// <remarks>Beware of unloading the target AppDomain in a continuation of this Task, as it's likely running with the AppDomain in the stack, causing a strange ThreadAbortException</remarks>
		public static implicit operator Task<TResult>(RemoteTask<TResult> remoteTask) => CreateTask(remoteTask);
		...
		public static implicit operator RemoteTask<TResult>(Task<TResult> task) => new RemoteTask<TResult>(task);
