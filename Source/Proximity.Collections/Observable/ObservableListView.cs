...
namespace System.Collections.Observable
{
	...
	{	//****************************************
		...
		public ObservableListView(IList<T> source) : this(source, (IComparer<T>?)null, null, null)
		{
		}
		...
		public ObservableListView(IList<T> source, int? maximum) : this(source, (IComparer<T>?)null, null, maximum)
		{
		}
		...
		public ObservableListView(IList<T> source, IComparer<T>? comparer) : this(source, comparer, null, null)
		{
		}
		...
		/// <param name="comparer">The comparer to use for sorting</param>
		/// <param name="maximum">The maximum number of items to show</param>
		public ObservableListView(IList<T> source, IComparer<T>? comparer, int? maximum) : this(source, comparer, null, maximum)
		{
		}
		...
		public ObservableListView(IList<T> source, Predicate<T> filter) : this(source, (IComparer<T>?)null, filter, null)
		{
		}
		...
		/// <param name="filter">A filter to apply to the source list</param>
		/// <param name="maximum">The maximum number of items to show</param>
		public ObservableListView(IList<T> source, Predicate<T> filter, int? maximum) : this(source, (IComparer<T>?)null, filter, maximum)
		{
		}
		...
		/// <param name="comparer">The comparer to use for sorting</param>
		/// <param name="filter">A filter to apply to the source list</param>
		public ObservableListView(IList<T> source, IComparer<T>? comparer, Predicate<T>? filter) : this(source, comparer, filter, null)
		{
		}
		...
		/// <param name="filter">A filter to apply to the source list</param>
		/// <param name="maximum">The maximum number of items to show</param>
		public ObservableListView(IList<T> source, IComparer<T>? comparer, Predicate<T>? filter, int? maximum)
		{
			_Source = source;
			Filter = filter;
			Maximum = maximum;
			Items = new List<T>(source.Count);
		...
		public int BinarySearch(T item) => Items.BinarySearch(0, Count, item, Comparer);
		...
		/// <remarks>Only searches the visible items</remarks>
		public bool Contains(T item) => Items.BinarySearch(0, Count, item, Comparer) >= 0;
		...
		public void CopyTo(T[] array, int arrayIndex) => Items.CopyTo(0, array, arrayIndex, Count);
		...
		protected virtual void OnItemsReset(T[] oldItems)
		{
		}
		...
		protected virtual void OnItemReplaced(T newItem, T oldItem, int index)
		{
		}
		...
		protected virtual void ResortItem(T newItem, int oldIndex, int newIndex, bool hasChanged)
		{
		...
		private void OnAddCollectionItem(T newItem)
		{
		...
		private void OnRemoveCollectionItem(T oldItem)
		{
		...
		private void OnReplaceCollectionItem(T oldItem, T newItem)
		{
		...
		private void OnCollectionChanged(NotifyCollectionChangedAction action, T changedItem, int oldIndex, int newIndex)
		{
		...
		private protected void VerifyList()
		{
			...
			for (var SubIndex = 1; SubIndex < Items.Count; SubIndex++)
			{
		...
		private static IComparer<T> GetDefaultComparer()
		{
			if (!typeof(IComparable<T>).IsAssignableFrom(typeof(T)) && !typeof(IComparable).IsAssignableFrom(typeof(T)))
				throw new ArgumentException($"{typeof(T).FullName} does not implement IComparable or IComparable<>");
		...
		{ //****************************************
			private readonly ObservableListView<T> _Parent;

			private int _Index;
			//****************************************

			internal ValueEnumerator(ObservableListView<T> parent)
			{
				_Parent = parent;
				_Index = 0;
				Current = default!;
		...
		private sealed class FilterComparer : IComparer<T>
		{ //****************************************
			private readonly IComparer<T> _Comparer;
			private readonly Predicate<T> _Filter;
			//****************************************

			internal FilterComparer(IComparer<T> comparer, Predicate<T> filter)
			{
				_Comparer = comparer;
				_Filter = filter;
			}
