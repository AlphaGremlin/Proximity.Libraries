...
namespace System.Threading.Tasks
{
	...
	public readonly struct ValueTaskMultiWaiter
	{
		...
		public ValueTaskMultiWaiter Then(ValueTask task) => new((Tasks ?? ImmutableStack<ValueTask>.Empty).Push(task));
		...
		/// <returns>A new waiter that waits on all the supplied tasks</returns>
		public ValueTaskMultiWaiter Then(IEnumerable<ValueTask> tasks) => new((Tasks ?? ImmutableStack<ValueTask>.Empty).Push(tasks, out _));
	...
	public readonly struct ValueTaskMultiWaiter<T>
	{
		...
		/// <param name="task">The task to wait on</param>
		/// <returns>A new waiter that waits on all the supplied tasks</returns>
		public ValueTaskMultiWaiter<T> Then(ValueTask<T> task) => new((Tasks ?? ImmutableStack<ValueTask<T>>.Empty).Push(task), Count + 1);
		...
		/// <param name="tasks">The tasks to wait on</param>
		/// <returns>A new waiter that waits on all the supplied tasks</returns>
		public ValueTaskMultiWaiter<T> Then(IEnumerable<ValueTask<T>> tasks) => new((Tasks ?? ImmutableStack<ValueTask<T>>.Empty).Push(tasks, out var Added), Count + Added);
