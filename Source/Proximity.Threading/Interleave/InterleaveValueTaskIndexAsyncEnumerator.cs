...
namespace System.Threading.Tasks.Interleave
{
	...
	public sealed class InterleaveValueTaskIndexAsyncEnumerator<T> : IAsyncEnumerator<(ValueTask<T> result, int index)>, IInterleaveAsyncEnumerator<ValueTask<T>>
	{ //****************************************
		private InterleaveCore<ValueTask<T>> _Core;
		//****************************************

		internal InterleaveValueTaskIndexAsyncEnumerator(IEnumerable<ValueTask<T>> source, CancellationToken enumerableToken, CancellationToken enumeratorToken)
		{
		...
		/// </summary>
		public (ValueTask<T> result, int index) Current => (_Core.CurrentResult, _Core.CurrentIndex);
	}
}
