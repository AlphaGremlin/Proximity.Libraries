...
namespace Proximity.Utility.Collections
{
	...
	public abstract class PagedSetBase<TKey, TItem> : IComparer<PagedSetPage<TKey, TItem>>, IComparer<PagedSetItem<TKey, TItem>> where TKey : IComparable<TKey>
	{	//****************************************
		private readonly SortedSet<PagedSetPage<TKey, TItem>> _Pages;
		...
		{
			_Pages = new SortedSet<PagedSetPage<TKey, TItem>>(this);
		...
		{	//****************************************
			...
			if (SortedItems.Length == 0)
			{
				...
				{
					_Pages.Add(new PagedSetPage<TKey, TItem>(true, true, this));
				}
			...
			if (OverlapsWith == null)
			{
				...
				_Pages.Add(new PagedSetPage<TKey, TItem>(PageItems, isStart, isFinish, this));
		...
		/// <summary>
		/// Adds an item to the last page, if it's marked as Finished
		/// </summary>
		...
		/// <summary>
		/// Mark the first page as no longer started
		/// </summary>
		public void InvalidateStart()
		{
			...
			_Pages.Max.IsStart = false;
		}

		/// <summary>
		/// Mark the last page as no longer finished
		/// </summary>
		...
		int IComparer<PagedSetPage<TKey, TItem>>.Compare(PagedSetPage<TKey, TItem> x, PagedSetPage<TKey, TItem> y)
		{
			return x.Min.CompareTo(y.Min);
		}

		int IComparer<PagedSetItem<TKey, TItem>>.Compare(PagedSetItem<TKey, TItem> x, PagedSetItem<TKey, TItem> y)
		{
