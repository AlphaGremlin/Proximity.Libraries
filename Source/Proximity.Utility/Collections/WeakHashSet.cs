...
namespace Proximity.Utility.Collections
{
	...
	{ //****************************************
		...
		public WeakHashSet(IEnumerable<TItem> collection, IEqualityComparer<TItem> comparer, GCHandleType handleType)
		{
			...
			Array.Sort<SetItem>(_Values, Comparer<SetItem>.Default);
		}
		...
		{ //****************************************
			...
			Key = _Comparer.GetHashCode(item);
			...
			if (Index >= 0)
			{
				...
				for (;;)
				{
					try
					{
						...
						if (Current != null && _Comparer.Equals(Current, item))
		...
		{ //****************************************
			...
			int Index = Array.BinarySearch<SetItem>(_Values, 0, _Size, new SetItem(Key));
			TItem Current;
			//****************************************
		...
		{ //****************************************
			...
			public bool MoveNext()
			{
				for (;;)
				{
					...
					try
					{
						_Current = (TItem)Handle.Item.Target;
		...
		private struct SetItem : IComparable<SetItem>
		{ //****************************************
			...
			internal SetItem(TItem item, WeakHashSet<TItem> hashSet)
			{
				...
				HashCode = hashSet._Comparer.GetHashCode(item);
			}
