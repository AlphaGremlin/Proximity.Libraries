...
namespace Proximity.Utility.Threading
{
	...
	{	//****************************************
		...
		public Task Queue(Action action) => Queue(action, _Factory.CancellationToken);
		...
		/// <returns>The task that was created</returns>
		public Task Queue<TValue>(Action<TValue> action, TValue value) => Queue(action, value, _Factory.CancellationToken);
		...
		/// <param name="action">The action to execute at the end of the stream</param>
		/// <returns>The task that was created</returns>
		public Task<TResult> Queue<TResult>(Func<TResult> action) => Queue(action, _Factory.CancellationToken);
		...
		/// <param name="value">The value to pass to the task</param>
		/// <returns>The task that was created</returns>
		public Task<TResult> Queue<TValue, TResult>(Func<TValue, TResult> action, TValue value) => Queue(action, value, _Factory.CancellationToken);
		...
		public Task QueueTask(Func<Task> action) => QueueTask(action, _Factory.CancellationToken);
		...
		/// <returns>A task representing the completion of the returned task</returns>
		/// <remarks>The stream will not continue until the returned task has been completed</remarks>
		public Task<TResult> QueueTask<TResult>(Func<Task<TResult>> action) => QueueTask(action, _Factory.CancellationToken);
		...
		/// <returns>A task representing the completion of the returned task</returns>
		/// <remarks>The stream will not continue until the returned task has been completed</remarks>
		public Task QueueTask<TValue>(Func<TValue, Task> action, TValue value) => QueueTask(action, value, _Factory.CancellationToken);
		...
		/// <returns>A task representing the completion of the returned task</returns>
		/// <remarks>The stream will not continue until the returned task has been completed</remarks>
		public Task<TResult> QueueTask<TValue, TResult>(Func<TValue, Task<TResult>> action, TValue value) => QueueTask(action, value, _Factory.CancellationToken);
		...
		private sealed class CompletedTask : IStreamTask
		{
			...
			TaskAwaiter IStreamTask.GetAwaiter() => throw new InvalidOperationException("Cannot await completed task");

			Task IStreamTask.Task => Task.CompletedTask;
		}
		...
		{	//****************************************
			...
			private void ExecWithValue() => _Action(_Value);
		...
		{	//****************************************
			...
			private TResult ExecWithValue() => _Action(_Value);

			//****************************************

			internal static Func<TResult> Create(Func<TValue, TResult> action, TValue value) => new TaskValue<TValue, TResult>(action, value).ExecWithValue;
		}
		...
		{	//****************************************
			...
			TaskAwaiter IStreamTask.GetAwaiter() => ((Task)Task).GetAwaiter();
			...
			Task IStreamTask.Task => base.Task;
		}
