...
namespace Proximity.Collections.Tests
{
	...
	public class IndexedDictionaryTests
	{
		...
		{	//****************************************
			var MySeed = Environment.TickCount;
			var MyRandom = new Random(MySeed);
			var MyRecords = new IndexedDictionary<int, int>(1024);
		...
		{	//****************************************
			...
			var MyRecords = new IndexedDictionary<int, int>(MyDictionary);
		...
		{	//****************************************
			...
			Console.WriteLine(MySeed); //597847015
		...
		[Test()]
		[TestCase(597847015)]
		public void RemoveAllSeed(int seed)
		{ //****************************************
			...
			CollectionAssert.AreEquivalent(MyDictionary, MyRecords, "Collections don't match");

			foreach (var MyPair in MyDictionary)
			{
				var Index = MyRecords.IndexOfKey(MyPair.Key);

				if (Index == -1)
					Assert.Fail();

				Assert.AreEqual(MyPair.Value, MyRecords.Get(Index).Value);
			}
